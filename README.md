...
An 8-byte shuffle bag: uniform random sampling without replacement for items <=64, with bitmask subset queries and draws. Includes a standard 54-card deck preset behind the `cards` feature.
...
(`cargo run --example cards --features=cards`) for a full game-style walkthrough.
